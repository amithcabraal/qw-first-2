...
function App() {
  ...
  const getEventsWithinTimeRange = useCallback((baseEvent: Event, maxYearDiff: number = 2): Event[] => {
    ...
    return events.filter(event => {
      if (!event?.date || event === baseEvent || usedEvents.has(event.eventDescription)) return false;
  ...
  const getRandomEvent = useCallback((): Event | null => {
    const validEvents = events.filter(event => 
      event.date && 
      event.eventImage && 
      !usedEvents.has(event.eventDescription)
    );
    
    if (validEvents.length === 0) {
      // If we've used all events, reset the used events tracking
      setUsedEvents(new Set());
      return events.filter(event => event.date && event.eventImage)[0];
    }
  ...
  const setupNewRound = useCallback(() => {
    ...
    if (nearbyEvents.length > 0) {
      ...
      // Add both events to used events
      setUsedEvents(prev => new Set([...prev, firstEvent.eventDescription, secondEvent.eventDescription]));
    } else {
  ...
  const startGame = useCallback(() => {
    ...
    setTimeLeft(60);
    setGameStarted(true);
    setUsedEvents(new Set()); // Reset used events when starting new game
    setupNewRound();
  }, [setupNewRound]);
